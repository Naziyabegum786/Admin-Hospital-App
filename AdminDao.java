...
public Admin verify(long phone, String password) {
	Query q=manager.createQuery("select a from Admin a where a.phone=?1 and a.password=?");
...
public Admin verify(String email, String password) {
	Query q=manager.createQuery("select a from Admin a where a.email=?1 and a.password=?");
